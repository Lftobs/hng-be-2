...
def test_register_user_success(db):
    ...
    assert response.json()["data"]["user"]["email"] == "john.doe777@example.com"
...
def test_create_organisation():
    ...
    response = client.post("/api/organisations", json={
        "name": "New Organisation8",
        "description": "A new organisation"
    }, headers={"Authorization": f"Bearer {access_token}"})
    assert response.status_code == 201
    assert response.json()["data"]["name"] == "New Organisation8"
